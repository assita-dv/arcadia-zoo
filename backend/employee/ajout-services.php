...
if (isset($_POST['send'])) {
    ...
    if (!empty($_POST['service_id']) && !empty($_POST['nom']) && !empty($_POST['description_servces'])) {
        ...
        if (mysqli_query($conn, $sql)) {
            header("location:services-db-emp.php?message=AddSuccess");
        } else {
            header("location:ajout-services.php?message=AddFail");
        }
    } else {
        // Redirection si les champs sont vides
        header("location:ajout-services.php?message=EmptyFields");
    }
}
?>
...
<body>
    <form action="" method="post">
        ...
        <a class="link black" href="/backend/employee/services-db-emp.php">Annuler</a>
    </form>
</body>
</html>
